...
xvfb-run --server-num 99$$ --auto-servernum -e $XVFB_ERR bash -c 'echo DISPLAY=$DISPLAY 1>&2; node /pdbe-images/build/index.js $ARGUMENTS'
# xvfb-run --server-num 99$$ --auto-servernum -f $XVFB_FILE -e $XVFB_ERR bash -c 'echo DISPLAY=$DISPLAY 1>&2; node /pdbe-images/build/index.js $ARGUMENTS'
